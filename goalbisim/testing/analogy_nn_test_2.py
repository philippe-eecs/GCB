...
def nearest_neighbor_analogy2(env, eval_transforms, device, replay_buffer, representation, step, details, samples = 5, k = 1, training_count = 1500, train_set = False, eval_replay_buffer = None, forced_samples = None):
	...
	video = MultiVideoRecorder(dir_name = details['eval_analogy_save_dir'], width = 2, fps = 9)
	video.init(num_trajectories = (k + 1) * samples, max_trajectory_length = env._max_episode_steps + 1)
	...
	for i in range(training_count // 500):
		...
		z = np.concatenate([representation(obses).detach().cpu().numpy(), representation(goals).detach().cpu().numpy()], axis = 1)
	...
	for i in range(samples):
		...
		z = np.concatenate([representation(traj_obses.unsqueeze(0)).detach().cpu().numpy(), representation(traj_goals.unsqueeze(0)).detach().cpu().numpy()], axis = 1)
