...
def nearest_neighbor_analogy4(env, eval_transforms, device, replay_buffer, representation, step, details, samples = 5, k = 1, training_count = 1500, train_set = False, eval_replay_buffer = None, forced_samples = None):
	...
	for i in range(training_count // 500):
		...
		#context = representation.phi(analogy_obses, analogy_goals)
		z = representation.psi(obses).detach().cpu().numpy()
	...
	for i in range(samples):
		...
		context = representation.phi(kwargs['analogy_obses'][i].unsqueeze(0), kwargs['analogy_goals'][i].unsqueeze(0)).detach().cpu().numpy() - representation.phi(kwargs['analogy_goals'][i].unsqueeze(0), kwargs['analogy_goals'][i].unsqueeze(0)).detach().cpu().numpy()
		z = representation.psi(traj_obses.unsqueeze(0)).detach().cpu().numpy() + context #Add
	...
	video.save(start + "/video_nn_analogies_complex", step)