...
class SawyerRigAffordancesV5(SawyerBaseEnv):
    ...
    def _load_table(self):
        ...
        if self.force_state:
            ...
            while(True):
                ...
                drawer_frame_pos = np.array([random.uniform(self.drawer_bounding_x[0], self.drawer_bounding_x[1]), random.uniform(self.drawer_bounding_y[0], self.drawer_bounding_y[1]), -.22])
        ...
        # Bottom Drawer
        bottom_drawer_frame_pos = drawer_frame_pos
        bottom_drawer_frame_pos[2] = -.34
        self._bottom_drawer = bullet.objects.drawer_no_handle(quat=quat, 
                pos=bottom_drawer_frame_pos, rgba=self.sample_object_color())
        self.bottom_drawer_open = random.uniform(0, 1) < .5
        ...
        # Button
        self.button_pos = bottom_drawer_frame_pos
        self.button_pos[2] = -.14
    ...
    def step(self, *action):
        ...
        if self.button_pressed():
            sign = 1 if self.bottom_drawer_open else -1
    ...
    def get_success_metric(self, curr_state, goal_state, key=None, success_list=None):
        if key == 'top_drawer':
            i, j = 8, 11
        ...
        if key == 'button':
            success = int((np.linalg.norm(curr_pos - goal_pos) < self.button_thresh))
        else:
    ...
    def get_contextual_diagnostics(self, paths, contexts):
        ...
        for i in range(len(paths)):
            ...
            td_success = self.get_success_metric(curr_obs, goal_obs, key='top_drawer', success_list=top_drawer_success_list)
        ...
        for i in range(len(paths)):
            curr_obs = paths[i]["observations"][-1][state_key]
            goal_obs = contexts[i][goal_key]
            td_success = self.get_success_metric(curr_obs, goal_obs, key='bottom_drawer', success_list=bottom_drawer_success_list)
            reward_list.append(td_success)
    ...
    def get_reward(self, info=None, print_stats=False):
        ...
        reward = 0
        if self.curr_task == 'button':
            bd_success = self.get_success_metric(curr_state, self.goal_state, key='bottom_drawer')
            button_success = self.get_success_metric(curr_state, self.goal_state, key='button')
        ...
        if print_stats:
            ...
            print('Bottom Drawer: ', bd_success)
        return reward

    def sample_goals(self):
        self.update_drawer_goal()
        self.update_button_goal()
        self.update_goal_state()

    def reset(self, seed = None, force_state = False, dont_change = False):
        if not force_state:
            if (seed != self.prev_seed) and not dont_change:
                self.prev_seed = seed
    ...
    def get_observation(self):
        ...
        bottom_drawer_pos = get_drawer_bottom_pos(self._bottom_drawer)
        button_height = [get_button_cylinder_pos(self._objects['button'])[2]]

        #(hand_pos, hand_theta, gripper, td_pos, bd_pos, b_pos)
        #(3, 4, 1, 3, 3, 1)
        observation = np.concatenate((end_effector_pos, hand_theta, gripper_tips_distance, top_drawer_pos, bottom_drawer_pos, button_height))
    ...
    def update_button_goal(self):
        ld_pos = self.get_object_pos('bottom_drawer')
        ld_frame_pos = get_drawer_frame_pos(self._bottom_drawer)
        self.bd_goal = ld_pos
        bd_open_pos = ld_frame_pos + bd_open_coeff * np.array([np.sin(self.drawer_yaw * np.pi / 180) , -np.cos(self.drawer_yaw * np.pi / 180), 0])
        bd_close_pos = ld_frame_pos + bd_close_coeff * np.array([np.sin(self.drawer_yaw * np.pi / 180) , -np.cos(self.drawer_yaw * np.pi / 180), 0])

        
        if np.linalg.norm(ld_pos[:2] - bd_close_pos[:2]) < self.drawer_thresh:
            self.bd_goal = bd_open_pos
        else:
            self.bd_goal = bd_close_pos

        self.button_goal = self.init_button_height - 0.01

    def update_goal_state(self):
        self.goal_state = np.concatenate([[0 for _ in range(8)], self.td_goal, self.bd_goal, [self.button_goal]])
    ...
    def get_demo_action(self, first_timestep=False, final_timestep=False):
        ...
        action, done = self.task_dict[self.curr_task]() #self.move_drawer()
    ...
    def press_button(self):
        ...
        target_pos = self.get_object_pos('button') + np.array([0, -0.0375, 0])
        aligned = np.linalg.norm(target_pos[:2] - ee_pos[:2]) < 0.02
        above = ee_pos[2] >= -0.101
        self.grip = -1.

        ## For finding coeff
        # drawer_pos = np.array(get_drawer_bottom_pos(self._bottom_drawer))
        #drawer_frame_pos = np.array(get_drawer_frame_pos(self._bottom_drawer))
        #print((drawer_pos - drawer_frame_pos)/np.array([np.sin((self.drawer_yaw+180) * np.pi / 180) , -np.cos((self.drawer_yaw+180) * np.pi / 180), 0]))
